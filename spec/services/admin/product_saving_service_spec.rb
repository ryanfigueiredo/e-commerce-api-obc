  ...
  context "when #execute" do
    context "sending loaded product" do
      ...
      context "with invalid :product params" do
        ...
        it "sets validation :errors" do
          service = error_proof_execute(product_params, product)
          expect(service.errors).to have_key(:name)
        end

        it "doesn't update :product" do
          expect {
            error_proof_execute(product_params, product)
            product.reload
      ...
      context "with invalid :productable params" do
        ...
        it "sets validation :errors" do
          service = error_proof_execute(game_params, product)
          expect(service.errors).to have_key(:developer)
        end

        it "doesn't update :productable" do
          expect {
            error_proof_execute(game_params, product)
    ...
    context "without loaded product" do
      ...
      context "with invalid :product params" do
        ...
        it "sets validation :errors" do
          service = error_proof_execute(params)
          expect(service.errors).to have_key(:name)
        end

        it "does not create a new product" do
          expect {
            error_proof_execute(params)
      ...
      context "without :productable params" do
        ...
        it "does not create a new product" do
          expect {
            error_proof_execute(product_params)
          }.to_not change(Product, :count)
        end

        it "sets validation :errors" do
          service = error_proof_execute(product_params)
...
def error_proof_execute(*params)
  service = described_class.new(*params)
  begin
    service.execute
  rescue => e
